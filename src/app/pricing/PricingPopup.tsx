...
export default function PricingPopup({ onClose }: PricingPopupProps) {
    ...
    const handleClose = () => {
        setShowModal(false);
        // Wait for animation to finish (300ms) before unmounting
        setTimeout(onClose, 300);
    };

    return (
        <div 
            className={`fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300 ease-out ${
                showModal ? "opacity-100" : "opacity-0"
            }`}
        >
            {/* Modal Container */}
            {/* Added transition-all, duration, and conditional scale/opacity classes */}
            <div className={`bg-background w-full max-w-6xl max-h-[87dvh] md:max-h-[90dvh] rounded-2xl shadow-2xl overflow-hidden flex flex-col relative border border-border transition-all duration-300 ease-out transform ${
                ...
                <div className="overflow-y-auto p-6 md:p-10 custom-scrollbar">
                    <div className="text-center mb-10">
                        ...
                        <div className="relative inline-flex items-center gap-2 md:gap-4 select-none">
                            <span
                                className={`text-md font-medium cursor-pointer transition-colors ${!isAnnual ? "text-primary font-bold" : "text-text-secondary"
                                    }`}
                            ...
                            >
                                <div
                                    className={`absolute top-[3px] w-5 h-5 bg-primary rounded-full shadow-md transition-all duration-300 ${isAnnual ? "left-[27px] md:left-[31px]" : "left-1"
                                        }`}
                                ></div>
                            </div>
                            ...
                            <span
                                className={`md:absolute left-full top-1/2 md:-translate-y-1/2 md:whitespace-nowrap text-xs font-bold text-success bg-success/10 px-2.5 py-0.5 rounded-full transition-all duration-300 ease-out ${isAnnual
