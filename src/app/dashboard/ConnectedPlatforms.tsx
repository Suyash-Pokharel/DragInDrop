    ...
    <section className="bg-gradient-to-br from-surface to-background border border-border p-6 md:p-8 rounded-[2rem] shadow-sm relative overflow-hidden group hover:border-primary/30 transition-colors">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          ...
          className="text-sm font-semibold text-primary hover:text-secondary flex items-center gap-1 group-hover:underline"
        >
          Manage All{" "}
      ...
      {socialAccounts.length === 0 ? (
        <div className="bg-surface border border-dashed border-border rounded-xl p-8 text-center flex flex-col items-center justify-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1 max-w-md">
              Connect your TikTok, YouTube, Instagram, or Threads accounts to start scheduling
              videos.
            </p>
          </div>
          <Link
            href="/settings/social-accounts"
            className="mt-2 text-sm font-bold bg-text-main text-surface px-6 py-2.5 rounded-xl hover:scale-105 transition-transform shadow-glow"
          >
            Connect Platforms
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {socialAccounts.map((acc) => (
            <div
              key={acc.id}
              className="group/acc relative p-4 bg-surface-highlight/50 hover:bg-surface-highlight border border-border hover:border-primary/30 rounded-xl transition-all duration-300"
            >
              <div className="absolute top-3 right-3">
                <span
                  className={`block w-2.5 h-2.5 rounded-full shadow-sm ${acc.isActive ? "bg-success shadow-[0_0_8px_var(--success)]" : "bg-error shadow-[0_0_8px_var(--error)]"}`}
                ></span>
              </div>
              <div
                className={`w-12 h-12 rounded-xl mb-3 flex items-center justify-center ${acc.isActive ? "bg-background text-text-main shadow-sm" : "bg-error/10 text-error"}`}
              >
