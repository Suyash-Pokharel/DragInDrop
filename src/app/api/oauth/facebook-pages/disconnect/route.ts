...
export async function POST(request: NextRequest) {
  ...
  // Authenticate user
  //  Return HTTP 403 if validation fails (user not authenticated)
  ...
  try {
    ...
    //  Query SocialAccount where userId matches and platform is "FacebookPage"
    //  Validate user owns the SocialAccount
    ...
    //  Return HTTP 404 if no active account found
    if (!socialAccount) {
    ...
    //  Set isActive to false (preserve historical data)
    ...
    try {
      ...
      console.log("[POST /api/oauth/facebook-pages/disconnect] Notification created successfully:", {
        userId: user.id,
        platform,
        timestamp: new Date().toISOString(),
      });
      ...
      console.error("[POST /api/oauth/facebook-pages/disconnect] Failed to create notification:", {
        userId: user.id,
        platform,
        timestamp: new Date().toISOString(),
    ...
    //  Return HTTP 500 when database error occurs
