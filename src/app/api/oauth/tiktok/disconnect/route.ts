...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    // Optionally revoke tokens with TikTok
    // Requirements: 7.8, 7.9 - Revoke tokens, log error if fails but continue
    try {
      const accessToken = decryptToken(socialAccount.accessToken);
      const clientKey = process.env.TIKTOK_CLIENT_KEY;

      if (clientKey && accessToken) {
        console.log("[DELETE /api/oauth/tiktok/disconnect] Attempting token revocation:", {
          userId: user.id,
          timestamp: new Date().toISOString(),
        });
        ...
        try {
          ...
          if (fetchError instanceof Error && fetchError.name === 'AbortError') {
            console.warn("[DELETE /api/oauth/tiktok/disconnect] Token revocation timeout:", {
              userId: user.id,
              timestamp: new Date().toISOString(),
              error: "Request timeout",
            });
            // Continue with disconnection even if revocation times out
            throw new Error("Token revocation timeout");
          }
        ...
        if (!revokeResponse.ok) {
          ...
          console.error("[DELETE /api/oauth/tiktok/disconnect] Token revocation failed:", {
            userId: user.id,
            timestamp: new Date().toISOString(),
          ...
          console.log("[DELETE /api/oauth/tiktok/disconnect] Token revoked successfully:", {
            userId: user.id,
            timestamp: new Date().toISOString(),
          });
        }
      ...
      // Requirement: 7.9 - Log error if revocation fails but still mark account inactive
      console.error("[DELETE /api/oauth/tiktok/disconnect] Token revocation error:", {
        userId: user.id,
        timestamp: new Date().toISOString(),
