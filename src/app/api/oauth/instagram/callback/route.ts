 ...
 * Handles OAuth 2.0 callback from Instagram for account authorization
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Wrap in try-catch to prevent OAuth flow failure if notification creation fails
      try {
        const { title, description } = formatSocialAccountConnected(
          "Instagram",
          sanitizedProfile.username
        );
        ...
        console.log("[GET /api/oauth/instagram/callback] Notification created successfully:", {
          userId: user.id,
          platform: "Instagram",
          timestamp: new Date().toISOString(),
        });
        ...
        console.error("[GET /api/oauth/instagram/callback] Failed to create notification:", {
          userId: user.id,
          platform: "Instagram",
          timestamp: new Date().toISOString(),
