  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className="h-full min-h-[300px] w-full flex flex-col items-center justify-center border border-dashed border-border/50 rounded-2xl text-text-secondary mt-4 bg-surface/30 backdrop-blur-sm relative overflow-hidden group">
        ...
        <span className="text-xs text-text-secondary/70 z-10 mt-1">
          User registration trends will appear here once users sign up.
        </span>
      </div>
    );
  }
          ...
          >
            {chartData.map((_entry, index) => (
              <Cell
                ...
                fill={
                  activeIndex === index
                    ? "url(#registrationBarGradientHover)"
                    : "url(#registrationBarGradient)"
                }
                className="transition-all duration-300"
                style={{
                  filter:
                    activeIndex === index ? "drop-shadow(0px 0px 8px var(--primary))" : "none",
                }}
              />
            ))}
          </Bar>
